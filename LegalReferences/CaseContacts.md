# Case Contacts Registry

**Case:** *Yibiao Lu and Jie Hu v. Fire Insurance Exchange* — Court File No. 27-CV-26-11606
**Purpose:** Canonical contact information for every individual and entity identified in this matter. Minn. R. Civ. P. 26.01(a)(1)(A) requires disclosing "the name and, **if known**, the address and telephone number of each individual" a party may use to support its claims. This file is the single source for those values.

**Last updated:** September 6, 2026

> **How to use.** Fill in a blank here once and it flows to the Initial Disclosures, discovery responses, subpoenas, and deposition notices. Entries marked ✅ are confirmed. Entries marked ❓ still need a source.

---

## Plaintiffs

| Name | Role | Address | Phone | Email | Status |
|---|---|---|---|---|---|
| Yibiao Lu | Plaintiff, named insured | 17756 George Moran Dr., Eden Prairie, MN 55347 | 763-843-2859 | bill_y_lu@yahoo.com | ✅ |
| Jie Hu | Plaintiff, insured spouse | 17756 George Moran Dr., Eden Prairie, MN 55347 | 763-843-2860 | jie_h_hu@yahoo.com | ✅ |

---

## Contractors and consultants

### Legacy Restoration LLC ✅

| Field | Value |
|---|---|
| **Correct legal name** | **Legacy Restoration LLC** |
| **Do not use** | ~~Legacy Contracting~~ · ~~Legacy Contractor~~ — both are incorrect and appear in earlier case documents (see "Naming correction" below) |
| Address | 15350 25th Ave N, Unit 114, Plymouth, MN 55447 |
| Phone | (763) 290-6966 |
| Source | Client-provided, September 6, 2026. Corroborated by the company email domain `legacyrestorationllc.com`. |

| Individual | Role in this matter | Contact |
|---|---|---|
| Mary [LAST NAME ❓] | Contacted Plaintiffs on August 9, 2024 to ask whether recent storms had caused damage; the contact that led to discovery of the leaking windows | Via Legacy Restoration LLC, above |
| Jack [LAST NAME ❓] | Visited the Property on August 13, 2024; first independent observation of dents on the windows; assisted Plaintiffs in opening the claim | Via Legacy Restoration LLC, above |
| **Patrick Flaten** ✅ | Assisted with Plaintiffs' administrative complaints; helped file the June 18, 2025 Consumer Assistance Request with the Minnesota Attorney General's Office (ID 6260735278129023664) | Direct: 608-633-8110 · pflaten@legacyrestorationllc.com · c/o Legacy Restoration LLC, above |

### Pure Construction ✅

| Field | Value |
|---|---|
| Entity | Pure Construction (Pure Construction MN) |
| Address | 16471 Territorial Trl, Maple Grove, MN 55369-2223 |
| Primary phone | (763) 307-3826 |
| Company email | info@pureconstructionmn.com |
| Source | Pella quote #19678463, `EventDocuments/CommunicationWithPureConstruction/17756 George Moran Drive,EP- Lu Bill - RESERVE TRAD-…-7-1-25.pdf` p. 1 |

| Individual | Role | Contact | Status |
|---|---|---|---|
| **Zach Sexe** | Primary contact; developed the scope submitted to appraisal; identified the discontinued Semco units and comparable current-production equivalents; retained John Foster; instructed Plaintiffs on September 12, 2025 not to speak with the appraisers | (952) 292-8004 · zach.pureconstruction@gmail.com · c/o Pure Construction, above | ✅ |
| **Dan Traxler** | Window-replacement scope and estimate | 507-317-6839 · c/o Pure Construction, above | ✅ |
| Jade [LAST NAME ❓] | Attended/handled the September 12, 2025 appraisal | c/o Pure Construction, above | ❓ |
| Records custodian | Scope-development file, appraisal submission, estimate workpapers | c/o Pure Construction, above | ✅ |

> **Note.** The Pella quote lists Pure Construction's mobile as "(952) 292-8004 ext. Zach S" — the same number the client supplied for Zach Sexe on September 6, 2026. Independently corroborated.

### Pella Window and Door Showroom of Plymouth — *reference only, not disclosed*

| Field | Value |
|---|---|
| Address | 13810 24th Avenue North, Suite 430, Plymouth, MN 55441 |
| Phone | (952) 915-6080 |
| Source | Pella quote #19678463 p. 1 |

| Individual | Role | Contact | Status |
|---|---|---|---|
| **Tim Conyard** | Pella sales representative who prepared **quote #19678463 (June 26, 2025)** — the document underlying the $303,820.78 window-replacement computation | 612-237-6392 · conyardt@pellamn.com | ✅ |

> **NOT disclosed as a witness — decision of September 6, 2026.** Plaintiffs never dealt with Conyard directly; Zach Sexe contacted Pella and compiled the estimate, and Zach is the sponsoring witness for it. Rule 26.01(a)(1)(A) reaches only individuals a party may use affirmatively. A damages expert may rely on quote #19678463 under Minn. R. Evid. 703 without Conyard testifying, and Rule 26.05 supplementation stays available if Defendant attacks the estimate's pricing. Contact details retained here only so the position can be revisited. See `Filings/Discovery/InitialDisclosure/Proposed_Changes_Initial_Disclosures_2026-09-06.md` P-2.

---

## The § 65A.01 appraisal panel

| Individual | Role | Contact | Status |
|---|---|---|---|
| John Foster | Plaintiffs' appraiser; signed the award; retained through Pure Construction at $2,200 (Plaintiffs paid $1,100) | 715-417-2307 · jjlmds@gmail.com | ✅ |
| Wade Roos | Umpire; signed the award; produced no report or estimate | ❓ — **not on the award form**, which carries no contact information for any panel member. Look in: the appraisal correspondence, Pure Construction's file, or Defendant's production. No public listing found. | ❓ |
| Greg Tomes | Defendant's appraiser; joined in appointing the umpire but **did not certify** the award amounts (see `OffenseStrategies/AppraisalAwardStrategy/09_AwardForm_Language_Findings.md` § 2) | ❓ — likely independent contractor; Rule 45 subpoena, or party notice if Defendant stipulates. Scheduling may be coordinated through Defendant's counsel | ❓ |

---

## Defendant and its vendors

**Fire Insurance Exchange**, by and through its attorney-in-fact Fire Underwriters Association (d/b/a Farmers Insurance)
**Counsel of record:** Kennedy Law Firm, Woodbury, MN — Kevin Kennedy and Mary Baskfield

> **Party vs. non-party — this distinction controls how each witness is reached.** Defendant's own employees are reached **through Defendant's counsel** (Minn. R. Prof. Conduct 4.2 cmt. [7]) and deposed on **notice** under Minn. R. Civ. P. 30. Vendors Defendant *retained* — Hancock Claims Consultants, Prieve Engineering LLC — are separate businesses, not Defendant's constituents; they are contacted at their own places of business and reached by **subpoena under Rule 45**. Comment [7] also states that consent of the organization's lawyer "is not required for communication with a **former** constituent," which matters for Allen Banning.

| Individual | Role | Contact |
|---|---|---|
| Christian Dudley | Catastrophe Claims Representative; signed the September 3, 2024 denial | Through Defendant's counsel |
| Nicholas Brau | Senior Claim Adjuster; September 6, 2024 inspection; February 10, 2025 voicemail | Through Defendant's counsel |
| Bradlee Monson | Field Claim Supervisor; February 3, 2025 "engineer's report only" instruction | Through Defendant's counsel |
| Zorphion Miller | Post-appraisal correspondent, September–October 2025 | Through Defendant's counsel |
| Twonda Miller | Post-appraisal correspondent, September–October 2025 | Through Defendant's counsel |
| Jed Jones | Post-appraisal correspondent, September–October 2025 | Through Defendant's counsel |
| Allen Banning | Adjusted the 2020 claim (3013539499-1) without an on-site inspection | Through counsel **if still employed**; if a former employee, contactable directly and a Rule 45 witness — employment status ❓ |
| Rule 30.02(f) designee(s) | To be designated | Through Defendant's counsel |

### Non-parties retained or engaged by Defendant — Rule 45 subpoena, own place of business

| Entity / individual | Role | Contact |
|---|---|---|
| **Peter Pietila** ✅ | Farmers agent — *likely independent agency; confirm the agency relationship in discovery* | 3701 Shoreline Dr, Ste 104A, Wayzata, MN 55391-9543 · 612-220-0342 (office) |
| **Prieve Engineering LLC — Nathan E. Prieve, P.E.** ✅ | October 9, 2024 inspection; report File #241008 and Photo Appendix A | 4937 Columbus Avenue, Minneapolis, MN 55417 · 612-203-2745 (call or text) · nathan@prieve-engineering.com · **MN P.E. License No. 46203** |
| Hancock Claims Consultants — **Jayden Grimm** ✅ | August 29, 2024 inspection; "Damaged – No" photograph set | **507-327-2595 (direct)** · c/o Hancock Claims Consultants, 6875 Shiloh Road East, Alpharetta, GA 30005 · (770) 569-1669 |
| Hancock Claims Consultants — **records custodian** ✅ | Complete inspection workproduct | 6875 Shiloh Road East, Alpharetta, GA 30005 · (770) 569-1669 · fax (678) 343-9608 |

---

## Regulators and records custodians

| Entity / individual | Role | Contact |
|---|---|---|
| **Katherine Toomajian, Minnesota Department of Commerce** ✅ | Plaintiffs' complaint ID 93547 | 85 7th Place East, Suite 280, St. Paul, MN 55101 · 800-657-3602 (toll free) · direct line ❓ |
| City of Eden Prairie — building/permit records custodian | Permit records for the Property and the neighborhood, including EP187829 (17490 George Moran Dr.) and EP190835 (17708 George Moran Dr.) | Verify: 8080 Mitchell Road, Eden Prairie, MN 55344 |
| National Weather Service / NOAA — records custodian | Weather records for July 13–14, 2024 | ❓ |

---

## Not disclosed as witnesses — by decision

| Group | Decision |
|---|---|
| Tim Conyard (Pella sales representative, quote #19678463) | **Omitted from the Rule 26.01(a) disclosures** by decision of September 6, 2026. Plaintiffs never dealt with him; Zach Sexe obtained the quote and compiled the estimate and is the sponsoring witness. A damages expert may rely on the quote under Minn. R. Evid. 703. Rule 26.05 supplementation remains available. |
| The 14 neighboring homeowners from the February 4–7, 2025 canvass, and the homeowners at 17490 and 17708 George Moran Dr. | **Omitted from the Rule 26.01(a) disclosures** by decision of September 6, 2026. Rule 26.01(a)(1)(A) reaches only witnesses a party may use affirmatively; hail causation is carried by the September 12, 2025 appraisal award, the NWS/NOAA records, the stormersite data, and the post-September 13, 2021 downspout dents. The canvass **document** is retained for the refused-review purpose only. Rule 26.05 supplementation remains available. See `Filings/Discovery/InitialDisclosure/Proposed_Changes_Initial_Disclosures_2026-09-06.md` P-2, P-3A, D-4. |

---

## Naming correction — Legacy Restoration LLC

The correct name is **Legacy Restoration LLC**. Earlier case documents use "Legacy Contracting," "Legacy contractor," or bare "Legacy."

**Corrected (September 6, 2026)** — working analysis files: `ClaimTimeline.md`, `DocumentIndex.md`, `LegalReferences/LegalArguments/A1.md` and `D2.md`, `OffenseStrategies/Discovery/DiscoveryPlan.md`, `OffenseStrategies/ForFamily/OurOffensiveWitnesses.md` and `PreparingForFarmersQuestions.md`, `OffenseStrategies/KeyFacts/key-facts-excluding-legal-context.md`, `OffenseStrategies/Motions/Daubert/MemorandumInSupport.md`, `LegalStrategies/bowl.md`, and the initial-disclosures review memo.

**Deliberately NOT corrected** — these are records of what was actually said or filed, and altering them would falsify the record:

| Document | Why it stands |
|---|---|
| `Filings/Complaint_Lu_Hu_v_Fire_Insurance_Exchange_FINAL.docx` and the filed PDFs (¶ 13 refers to "Mary of Legacy contractor" and "Jack of Legacy") | **The Complaint was filed July 8, 2026.** The working copy must continue to match the court's record. Correcting the name in the pleading requires an amendment under Minn. R. Civ. P. 15, or simply clarifying the entity's full name in discovery — not a silent edit. This is a naming imprecision, not a misstatement of fact, so an amendment for this reason alone is likely unnecessary. |
| `OfficialCourtDocuments/…Complaint-Civil….pdf`, `…Answer….pdf` | Court records. |
| `Filings/SummonsAndComplaint/AnswerFromFIE/…` and the Expert Record Packet copies | Defendant's Answer quotes the Complaint's language back. |
| `EventDocuments/Copy of ComunicationLogs.docx.md` | Plaintiffs' contemporaneous communication log — an evidentiary record of what was written at the time. |
| `EventDocuments/2025-01-31-ResponseToFarmer.pdf`, `2025-02-10-SecondResponseToFinalReport.pdf`, `11-19-complaint-to-dept-of-commerce.pdf` | Correspondence as sent to Farmers and to the Department of Commerce. |
| `LegalStrategies/Draft_Complaint_v1/v2/V4.md` | Superseded drafts; part of the drafting history. |
| `LegalStrategies/APPENDIX_exhibits.pdf`, `Appendix_B_Exhibits.pdf` | Exhibit compilations. |

**Going forward:** use "Legacy Restoration LLC" on first mention in any new document, and "Legacy" as short form thereafter. If Defendant raises the discrepancy between the Complaint's "Legacy contractor" and the disclosures' "Legacy Restoration LLC," the answer is that they are the same entity and the disclosures state its correct legal name.


---

## Sources for confirmed entries

| Entry | Source |
|---|---|
| Legacy Restoration LLC; Patrick Flaten | Client-provided, September 6, 2026; company email domain `legacyrestorationllc.com` |
| Pure Construction entity; Zach Sexe mobile | Pella quote #19678463 p. 1 (`EventDocuments/CommunicationWithPureConstruction/…FULL FRAME INSTALL-7-1-25.pdf`); client-confirmed September 6, 2026 |
| Zach Sexe direct / email; Dan Traxler phone; Peter Pietila agency | Client-provided, September 6, 2026 |
| John Foster | Appraisal Demand, per `DocumentIndex.md` |
| Nathan Prieve — license number | Prieve report File #241008, certification page (`EventDocuments/FarmersDenyLetter-11-4.pdf`) |
| Nathan Prieve — address, phone, email | [Prieve Engineering LLC contact page](https://sites.google.com/prieve-engineering.com/prieve-engineering/contact) |
| Dept. of Commerce address | Farmers' November 4, 2024 denial letter, § 72A.201 notice paragraph |
| Hancock Claims Consultants (corporate) | [Hancock Claims Consultants contact page](https://hancockclaims.com/contact/) |
| Jayden Grimm direct line; Peter Pietila agency | Client-provided, September 6, 2026 |
