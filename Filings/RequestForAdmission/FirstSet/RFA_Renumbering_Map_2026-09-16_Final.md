# RFA Final Renumbering Map — First Set to FIE (September 16, 2026)

**Built:** September 16, 2026, by the final renumbering pass (tracker item K-8 in `RFA_First_Set_Independent_Review_2026-09-16.md`).

**Maps:** the working numbering used during the September 16, 2026 review (slots 1–192, with 63, 97 and 98 vacant) → the **final numbering, Nos. 1–189**, in `Plaintiffs_First_Set_of_Requests_for_Admission_to_FIE_NoAttachment.md`.

**Pre-renumbering copy:** `[9-15 before re-numbering whole set] Plaintiffs_First_Set_of_Requests_for_Admission_to_FIE_NoAttachment.md` (the user's saved baseline), plus git history.
...
- **Moved appended sections next to their topics** and relettered all sections A–U in document order.
- **Two wording changes only:**
  - Final No. 164 (working 77) now cites "Request No. 162" (was No. 75).
  - Final No. 137 (working 188) now says "the statement quoted in the preceding request" (was "Request No. 60"), because it now directly follows final No. 136.
- **Everything else is unchanged:** every other request text is identical to its working version (checked by script), as are the preamble, definitions, instructions, signature blocks and certificate.
- **Relative references checked** ("the preceding request" / "the three preceding requests"): final Nos. 48, 80, 135, 137 and 163 each still point to the intended requests.
- **Section notes rewritten** to the final numbers:
  - F: Requests 47–48 / 49–50, with No. 46.
  - G: Requests 51–56, with Nos. 14, 38, 57–64.
  - I: Nos. 57–64.
  - J: Nos. 82–101 and 124–130.
  - N: Requests 141–150, with Nos. 8–12, 33, 136–138.
...
| Final № | Working № | Working section | Final section |
|---|---|---|---|
| **1** | 1 | A | A |
| **2** | 2 | A | A |
| **3** | 3 | A | A |
...
## Table B — working number → final number
...
- `Plaintiffs_First_Set_of_Requests_for_Admission_to_FIE_NoAttachment.md`: renumbered.
- `../SecondSet/Plaintiffs_Second_Set_of_Requests_for_Admission_to_FIE_DRAFT.md`: every First Set number converted to final numbering.
- `../../Discovery/InitialDisclosure/Plaintiffs_Initial_Disclosures.md` (drafter's note): Nos. 94–96 → **185–187**.
- `../../../OffenseStrategies/Deposition/2.1Prieve_Nathan.md`: RFA No. 153 → **103**.
- `../README.md`, `../../Discovery/Service_Log.md`: counts updated.
- `RFA_First_Set_Independent_Review_2026-09-16.md`: K-1, K-2, K-3 and K-8 marked done; banner added. **Its body and tracker still use working numbers; convert them with Table B.**
- `Proposed_Changes_RFA_First_Set_2026-09-12.md`: banner added (its status table uses working numbers).
- `RFA_Renumbering_Map_2026-09-15.md`: marked historical.
