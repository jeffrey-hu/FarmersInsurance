# DRAFT — Plaintiffs' Second Set of Requests for Production to FIE

**Case:** *Yibiao Lu and Jie Hu v. Fire Insurance Exchange* — Court File No. 27-CV-26-11606
**Court:** Hennepin County District Court, Fourth Judicial District (Judge Jamie L. Anderson)
**Created:** September 10, 2026
**Origin:** Request No. 33 of the First Set, removed from that set before service under item **P-21** of `../FirstSetDiscovey/Request For Production/Proposed_Changes_RFP_First_Set_2026-09-08.md`.

> ### ⚠️ DRAFT — NOT FOR SERVICE
> This set is **held deliberately**. It should not be served until the trigger conditions below are met. Serving it early is the specific harm P-21 was written to avoid.

> **Not legal advice.** Have a Minnesota-licensed attorney review before service.

---

## Why this set is held

The vendor-pattern request below is defensible on relevance — Complaint ¶72 pleads that Defendant's conduct "reflects institutional claim-handling practices affecting numerous Minnesota homeowners," which is a pleaded element of the Consumer Fraud Act count, not a bad-faith theory awaiting a § 604.18 amendment. But relevance is not the issue. **Sequence is.**

Served in the first round, it is the request Defendant builds a protective-order motion around, and it makes the narrowed vendor requests that *did* go out — First Set Nos. 9 (Prieve engagements) and 28 (Hancock engagements) — look like the same overreach in smaller type.

Served second, after Defendant has answered whether it performed any independent review of the vendor reports, it stops being a fishing expedition and becomes targeted follow-up on an established fact.

## Trigger conditions — serve only after all three

1. **Interrogatory 17** (First Set) is answered: Defendant's policies and practice for retaining, reviewing, evaluating, and accepting or rejecting third-party inspection and engineering reports, including who reviews them and what independent analysis is performed.
2. **Request 32** (First Set) is responded to: documents reflecting Defendant's review, evaluation, approval, or adoption of the Hancock and Prieve reports.
3. **Requests 9 and 28** (First Set) are responded to: the narrowed Minnesota residential hail engagement histories for Prieve and Hancock.

**What to look for in those answers.** If Defendant states or shows that it performed no independent review of the vendor reports — or produces an engagement history showing a lopsided causation-finding pattern — this set becomes follow-up on Defendant's own admission. If Defendant instead documents a genuine review process, reconsider whether this set is worth the fight at all.

## Timing against the schedule

Plaintiffs' proposed Scheduling Order sets **substantial completion of document production at February 26, 2027** and the **close of fact discovery at June 30, 2027**. First Set responses are due roughly 30 days after service. That leaves room for this set with a full 30-day response period and a meet-and-confer cycle before substantial completion — but only if it goes out promptly once the triggers are met. Calendar a check when the First Set responses arrive.

---

## Draft Request No. 1 — vendor management and oversight

**Preserved verbatim as it read in the First Set (No. 33), unedited:**

> **REQUEST FOR PRODUCTION NO. 1:** All vendor-management, selection, oversight, performance-evaluation, and service-agreement documents concerning Hancock Claims Consultants and Prieve Engineering, LLC from January 1, 2020 forward, including documents reflecting the volume of assignments to, and payments made to, each.

### Open decision before service: serve as drafted, or narrowed?

P-21 flagged this text as likely to draw an overbreadth objection and offered a narrowed alternative. **That narrowing was never approved** — the text above is preserved exactly as it stood. Decide at service time:

**Option A — as drafted above.** Broadest reach. Highest objection risk.

**Option B — narrowed:**

> Documents sufficient to show the vendor-selection criteria, performance metrics, and service agreements applicable to Hancock Claims Consultants and Prieve Engineering, LLC for Minnesota property claims from January 1, 2020 forward, together with the annual volume of assignments to and payments made to each.

**Option C — as drafted, but conditioned on what the First Set produced.** If Defendant's answers establish no independent review, Option A is far easier to defend, because the request is then aimed at a documented practice rather than a suspected one.

If either option is served, add the protective-order and third-party-redaction offer used in First Set Nos. 9 and 28 — it removed the confidentiality objection there and should do the same here.

---

## Other candidates for this set

Neither is approved; both are pending decisions in `../FirstSetDiscovey/Request For Production/Proposed_Changes_RFP_First_Set_2026-09-08.md`. Listed here so they are not lost.

| Source item | Subject | Status | Note |
|---|---|---|---|
| **P-31** | Hail-claim training and outcome-linked compensation for Brau, Monson, Dudley, and Pietila | Pending — recommended for this set | "Documents sufficient to show," four named people, no general personnel-file demand. Expect a third-party privacy objection; it is a better fight in round two than round one. |
| **P-32** | Claim reserve history and each change | Pending — recommended for **after** the § 604.18 motion to amend is granted | Weakest relevance hook while § 604.18 is unpleaded, and the strongest carrier resistance. The proposed Scheduling Order sets that motion deadline at November 1, 2027. |

---

## Before serving this set

- Carry over the First Set's Definitions and Instructions (11 instructions as applied, including the attorney-client carve-out, the possession-custody-or-control instruction, and the Rule 34.02 partial-objection instruction).
- Number this set's requests **starting at 1**, as a separate set — do not continue the First Set's numbering.
- Confirm the First Set's numbering is not disturbed: the First Set now runs **1 through 40** with no gap, because this request was removed **before service**.
